...
categoryRoutes.get("/", (req, res, next) => {
    const query = `SELEC id, name FROM category`
    db.query(query, (err, result, fields) => {
        //  console.log("🚀 ~ file: categoryRoutes.js:10 ~ db.query ~ result", result)
        //  console.log("🚀 ~ file: categoryRoutes.js:10 ~ db.query ~ fields", fields)
        if (err) return next(err)
        return res.status(200).send({ message: "Estas son las categorias que se encontraron", success: true, result })
    })
})
